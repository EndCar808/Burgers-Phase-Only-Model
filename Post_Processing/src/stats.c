...
void compute_stats() {
	...
	// D3dims[1] = IncLen;
	// D3dims[2] = 5;
	// if((H5Lexists(file_info->input_file_handle, "VelInc_Stats", H5P_DEFAULT)) <= 0 ) {
	// 	printf("Writing ""%s""...\n", "VelInc_Stats");
	// 	if((H5LTmake_dataset(file_info->input_file_handle, "VelInc_Stats", HDF_D3ndims, D3dims, H5T_NATIVE_DOUBLE, VelInc_Stats[0][0])) < 0 ) {
	// 		fprintf(stderr, "\nERROR in file:%s @line:%d | unable to make dataset %s in HDF5 file %s\n", __FILE__, __LINE__, "VelIncs_Stats", file_info->input_file_name);
	// 		exit(1);		
	// 	}
	// } else {
	...
	if((H5Lexists(file_info->input_file_handle, "Derivative", H5P_DEFAULT)) <= 0 ) {
		printf("Writing ""%s""...\n", "Derivative");
		if((H5LTmake_dataset(file_info->input_file_handle, "Derivative", HDF_D2ndims, D2dims, H5T_NATIVE_DOUBLE, stat_vars->dudx)) < 0 ) {
			fprintf(stderr, "\nERROR!! | in file:%s @line:%d | unable to make dataset %s in HDF5 file %s\n", __FILE__, __LINE__, "Derivative", file_info->input_file_name);
			exit(1);		
		}
	} else {
		printf("\nDidn't Write \"%s\", dataset already exists!\n", "Derivative");
	...
	// D2dims[0] = 1;
	// D2dims[1] = num_tsteps * N;
	// if((H5Lexists(file_info->input_file_handle, "VelocityIncrements", H5P_DEFAULT)) <= 0 ) {
	// 	printf("Writing ""%s""...\n", "VelocityIncrements");
	// 	if((H5LTmake_dataset(file_info->input_file_handle, "VelocityIncrements", HDF_D2ndims, D2dims, H5T_NATIVE_DOUBLE, stat_vars->du_r)) < 0 ) {
	// 		fprintf(stderr, "\nERROR!! | in file:%s @line:%d | unable to make dataset %s in HDF5 file %s\n", __FILE__, __LINE__, "VelocityIncrements", file_info->input_file_name);
	// 		exit(1);		
	// 	}
	// } else {
	// 	printf("\nDidn't Write \"%s\", dataset already exists!\n", "VelocityIncrements");
	...
	D2dims[0] = 1;
	D2dims[1] = num_tsteps * N;
	if((H5Lexists(file_info->input_file_handle, "VelIncSmall", H5P_DEFAULT)) <= 0 ) {
		printf("Writing ""%s""...\n", "VelIncSmall");
		if((H5LTmake_dataset(file_info->input_file_handle, "VelIncSmall", HDF_D2ndims, D2dims, H5T_NATIVE_DOUBLE, stat_vars->du_r_small)) < 0 ) {
			fprintf(stderr, "\nERROR!! | in file:%s @line:%d | unable to make dataset %s in HDF5 file %s\n", __FILE__, __LINE__, "VelIncSmall", file_info->input_file_name);
			exit(1);		
		}
	} else {
		printf("\nDidn't Write \"%s\", dataset already exists!\n", "VelIncSmall");
	...
	D2dims[0] = 1;
	D2dims[1] = num_tsteps * N;
	if((H5Lexists(file_info->input_file_handle, "VelIncMid", H5P_DEFAULT)) <= 0 ) {
		printf("Writing ""%s""...\n", "VelIncMid");
		if((H5LTmake_dataset(file_info->input_file_handle, "VelIncMid", HDF_D2ndims, D2dims, H5T_NATIVE_DOUBLE, stat_vars->du_r_mid)) < 0 ) {
			fprintf(stderr, "\nERROR!! | in file:%s @line:%d | unable to make dataset %s in HDF5 file %s\n", __FILE__, __LINE__, "VelIncMid", file_info->input_file_name);
			exit(1);		
		}
	} else {
		printf("\nDidn't Write \"%s\", dataset already exists!\n", "VelIncMid");
	...
	D2dims[0] = 1;
	D2dims[1] = num_tsteps * N;
	if((H5Lexists(file_info->input_file_handle, "VelIncLarge", H5P_DEFAULT)) <= 0 ) {
		printf("Writing ""%s""...\n", "VelIncLarge");
		if((H5LTmake_dataset(file_info->input_file_handle, "VelIncLarge", HDF_D2ndims, D2dims, H5T_NATIVE_DOUBLE, stat_vars->du_r_large)) < 0 ) {
			fprintf(stderr, "\nERROR!! | in file:%s @line:%d | unable to make dataset %s in HDF5 file %s\n", __FILE__, __LINE__, "VelIncLarge", file_info->input_file_name);
			exit(1);		
		}
	} else {
		printf("\nDidn't Write \"%s\", dataset already exists!\n", "VelIncLarge");
