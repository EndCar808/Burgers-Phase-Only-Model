...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < num_osc; ++i) {
	 	...
	 	phase_sync_ser[i] = 0.0 + 0.0 * I;
	 	phase_sync_par[i] = 0.0 + 0.0 * I;
	} 
	...
	for (int k = kmin; k <= kmax; ++k) {
		...
		for (int kk1 = 0; kk1 <= 2 * kmax - k; ++kk1) {
			...
			if( (abs(k1) >= kmin) && (abs(k - k1) >= kmin)) {
				...
				phase_sync_ser[k] +=  cexp(I * (sgn(k - k1) * phi[abs(k1)] + sgn(k1) * phi[abs(k - k1)] - sgn(k1 * (k - k1)) * phi[k])); //amps[abs(k1)] * amps[abs(k - k1)] * 
	...
	t_s = omp_get_wtime();
	t   = t_e - t_s; 
	printf("Time for Serial: %5.16lf || Speed up: %5.16lf\n", (t_s - t_e), t / (t_e - t_s));
	...
	// Define temporary (private) variable
	fftw_complex phase_tmp;

	// // Compute the ORDERED phase order parameter

	#pragma omp parallel shared(phase_sync_par, phi, kmin, kmax, phase_tmp)
	{
		for (int k = kmin; k <= kmax; ++k) {

			
			// Loop over shitfed k1 domain			
		#pragma omp for reduction(+:phase_tmp)
			for (int kk1 = 0; kk1 <= 2 * kmax - k; ++kk1) {
				// Readjust k1 to correct value
				k1 = kk1 - kmax + k;
				...
				// Consider valid k1 values
				if( (abs(k1) >= kmin) && (abs(k - k1) >= kmin)) {
					phase_tmp +=  cexp(I * (sgn(k - k1) * phi[abs(k1)] + sgn(k1) * phi[abs(k - k1)] - sgn(k1 * (k - k1)) * phi[k])); //amps[abs(k1)] * amps[abs(k - k1)] * 
				}			
			}
	...
	// Get wall time After 
	t_s = omp_get_wtime();
	printf("Time for Parallel: %5.16lf || Speed up: %5.16lf\n", (t_s - t_e), t / (t_e - t_s));
	...
	fftw_free(u_pad);
	fftw_free(u_z_pad);
	fftw_free(phase_sync_ser);
	fftw_free(phase_sync_par);
