...
&	set_timescale,convolve,convolve_fft,RealSpace, theoretical_energy, omega_max_min
	...
	FUNCTION convolve_fft(a_e_phi,N)
		...
		! DO j = 1,N_fftw_convolve/2+1
		! 	print*, "u_z_b[", j -1 , "]", x_cmplx_convolve(j)
		! END DO
		! print*, ""
		...
		! DO j = 1,N_fftw_convolve/2+1
		! 	print*, "u_z_a[", j -1 , "]", x_cmplx_convolve(j)
		! END DO
		! print*, ""
	...
	FUNCTION rk_4_integrate_fft(phi,a_k,omega_k,N,dt)
		...
		! rk_4_integrate_fft=phi+(dt*1.0/6.0)*k1+(dt*1.0/3.0)*k2+(dt*1.0/3.0)*k3+(dt*1.0/6.0)*k4
