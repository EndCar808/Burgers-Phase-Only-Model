...
if __name__ == '__main__':
	...
	for k in range(num_osc):
		print("Phases - k = {}".format(k))
		plt.plot(time[:], phases[:, k])
		plt.ylabel(r"$\phi_{{}}(t)$".format(k))
		plt.xlim(time[0], time[-1])
		plt.xlabel(r'$t$')
		plt.legend(r"$k = {}$".format(k))
		plt.savefig(output_dir + "/Phases_k[{}].png".format(k))  
		plt.close()


	for k1 in range(kmin, int(k/2) + 1):
		print("Triads - k1 = {}".format(k1))
		plt.plot(time[:], np.transpose(triads[:, k1 - kmin, :]))
		plt.ylabel(r"$\varphi_{k1, k_2}^{k3} (t)$")
		plt.xlim(time[0], time[-1])
		plt.title(r"$k_1 = {}$".format(k1))
		plt.xlabel(r'$t$')
		plt.savefig(output_dir + "/Triads_k1[{}].png".format(k))  
		plt.close()