...
def plot_snaps(i, x, u_urms, du_x_rms, time, triads, kmin, kmax, phases, R, Phi, t0, T, fullTime):
    ...
    plt.savefig(output_dir + "/SNAPS/Triad_SNAPS_{:05d}.pdf".format(i))  
    plt.close()
...
if __name__ == '__main__':
	...
	if dist[0] == 'ubuntu':
		...
		output_dir = "/ichec/home/users/endacarroll/Burgers/burgers-code/Data/RESULTS" + results_dir
	...
	plot_snaps(i, x, u_urms[i, :], du_x_rms[i, :], time[i], triads[:, :, i], kmin, kmax, phases[i, kmin:], R[0:i], Phi[0:i], time[0], time[-1], time[0:i])
	...
	# TRIADS PDF
	hist, bins   = np.histogram(np.extract(triads[:, :, :] != -10, triads).flatten(), bins = 500, density = True);
	bin_centers = (bins[1:]+bins[:-1])*0.5
	...
	plt.savefig(output_dir + "/LCE_DYNAMICS_N[{}]_ALPHA[{:0.3f}]_BETA[{:0.3f}]_k0[{}]_ITERS[{}].pdf".format(N, alpha, beta, k0, iters))  
	plt.close()
